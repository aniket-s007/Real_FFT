...
# ---------------------------------------------------------------------------
# Fixed test vectors -- no randomness, so results are reproducible and
# directly comparable against the RTL testbenches run-to-run.
#
# WIDTH/SCALE describe the Q1.(WIDTH-1) fixed-point format tb_stage1.v /
# tb_stage2.v quantize into via their own to_fixed() -- that quantization
# still happens at that later testbench stage, not here. SCALE is used
# only so EDGE_CASE_X's values land exactly on that format's
# representable corners (WIDTH=8 matches those testbenches' default).
# ---------------------------------------------------------------------------
WIDTH = 8
SCALE = 1 << (WIDTH - 1)   # matches SCALE in tb_stage1.v / tb_stage2.v

# The exact 16-sample frame already hardcoded in tb_stage1.v / tb_stage2.v's
# x_real[] array -- keep in sync if that array ever changes.
NOMINAL_X = [
    0.10, -0.25, 0.40, -0.55,
    0.65, -0.05, 0.30, -0.80,
    0.15, 0.45, -0.35, 0.60,
...
# Corners of the Q1.(WIDTH-1) format (WIDTH=8 -> representable range
# [-1.0, +127/128], LSB = 1/128): max/min representable, zero, +-1 LSB,
# +-0.5 LSB (to_fixed's round-half-away-from-zero tie point), a couple of
# exact binary fractions, and two out-of-range values that will need
# saturation/overflow handling once a later stage actually quantizes them.
EDGE_CASE_X = [
    (SCALE - 1) / SCALE,   # 0: max positive representable
    -1.0,                  # 1: min representable (exact -1.0)
    0.0,                   # 2: zero
    1.0 / SCALE,           # 3: +1 LSB, smallest positive step
    -1.0 / SCALE,          # 4: -1 LSB, smallest negative step
    0.5 / SCALE,           # 5: +0.5 LSB, exact rounding tie
    -0.5 / SCALE,          # 6: -0.5 LSB, exact rounding tie
    2.0 / SCALE,           # 7: +2 LSB
    -2.0 / SCALE,          # 8: -2 LSB
    0.5,                   # 9: exact half-scale, binary-exact fraction
    -0.5,                  # 10: exact negative half-scale
    0.25,                  # 11: exact quarter-scale, binary-exact fraction
    -0.25,                 # 12: exact negative quarter-scale
    1.5,                   # 13: out of range positive -> saturation/overflow probe
    -1.5,                  # 14: out of range negative -> saturation/overflow probe
    1.0 - 0.5 / SCALE,     # 15: just under +1.0, rounding tie into overflow
]

TEST_VECTORS = {
    "nominal (tb_stage1/tb_stage2 frame)": NOMINAL_X,
    "edge_case (fixed-point corners, WIDTH=8)": EDGE_CASE_X,
}
...
def main():
    ...
    for name, x in TEST_VECTORS.items():
        print(f"\n{'=' * 80}\nTest vector: {name}\n{'=' * 80}")
